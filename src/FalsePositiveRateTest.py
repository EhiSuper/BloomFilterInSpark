...
def getFalsePositiveRates(rdd, bloom_filters, bloom_parameters):
    ...
    false_positive_rates = false_positive_rates.flatMap(lambda x: getFalsePositives(x, bloom_parameters, bloom_filters)) #return a list of lists where each list is in the form [rating, 1] 
    #if the film is a false positive for the bloom filter related to that rating.
    false_positive_rates = false_positive_rates.reduceByKey(lambda a,b : a+b)
    false_positive_rates = false_positive_rates.map(lambda x: getFalsePositiveRate(x, bloom_parameters)).sortByKey()

    false_positive_rates.saveAsTextFile("Data/FalsePositiveRates")
